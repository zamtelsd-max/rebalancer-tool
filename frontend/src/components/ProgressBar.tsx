...
export default function ProgressBar({ label, pct, actual, target, weight, color = '#00843D' }: Props) {
  const capped = Math.min(pct, 100);
  return (
    <div className="space-y-1">
      <div className="flex justify-between text-xs font-medium text-gray-700">
        <span>{label}{weight ? <span className="text-gray-400 ml-1">({weight}%)</span> : ''}</span>
        <span className="font-bold" style={{ color: pct >= 80 ? '#00843D' : pct >= 60 ? '#ca8a04' : pct >= 40 ? '#ea580c' : '#dc2626' }}>{pct}%</span>
      </div>
